...
const fs = require("fs");
const { createDir, checkJSON, getData, addLinkWithError, errorText } = require("./functions");
...
const downloadAll = async (catalogFolder, downloadFolder) => {
	...
	for (const item of catalog) {
		...
		await createDir(downloadPath, `${catalogFolder}/downloadLinksWithError.json`);

		for (const imageLink of images) {
			try {
				await downloadFile(SKU, imageLink, downloadPath, catalogFolder, g);
				console.log(`Товар ${i} из ${catalog.length}\n Файл ${g} из ${images.length}`);
				g++;
			} catch (error) {
				addLinkWithError(imageLink, `${catalogFolder}/downloadLinksWithError.json`);
				console.error(errorText(error));
			}
		}

		await fs.promises.writeFile(downloadedLinksPath, JSON.stringify([...downloadedLinks, SKU]));
		i++;
	}
};
